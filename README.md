...
Base teórica: exclusivamente **Claudio Naranjo** (*Caráter e neurose* e a coleção
*Psicologia dos eneatipos*, um volume por paixão) e **Oscar Ichazo**.

O banco é escrito a partir de duas coisas do livro: os **clusters de traços** que Naranjo
lista para cada tipo (cada cluster precisa aparecer em pelo menos um item) e o **apêndice de
diagnóstico diferencial**, que dá, par a par, o que separa dois tipos parecidos. Os 25 pares
do apêndice estão cobertos por itens cruzados ou de desempate, e isso é verificado por teste.
